...
export default function ScrollShowcase({
  ...
  function goToItem(index: number) {
    ...
    const targetProgress = Math.min((index + 0.08) / items.length, 1);

    window.scrollTo({
      top: stageTop + scrollableDistance * targetProgress,
      behavior: "smooth",
    });
  }
  ...
  useEffect(() => {
    ...
    function updateFromScroll() {
      ...
      const nextIndex =
        items.length <= 1
          ? 0
          : Math.min(items.length - 1, Math.floor(nextProgress * items.length));
            ...
            <div className="showcase-track">
              <div className="showcase-track__inner">
                ...
                >
                  ...
                  <ul
                    className="tag-list"
                    aria-label={`${activeItem.title} topics`}
                  >
                ...
                {previewItem && previewIndex !== null && (
                  <article
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={`Show ${previewItem.title}`}
                    onClick={() => goToItem(previewIndex)}
                    onKeyDown={(event) => {
                      if (event.key === "Enter" || event.key === " ") {
                        event.preventDefault();
                        goToItem(previewIndex);
                      }
                    }}
                  >
                    <div className="showcase-item__meta">
